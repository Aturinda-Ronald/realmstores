...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($firstName) || empty($lastName)) {
        ...
        if ($stmt->fetch()) {
            setMessage('Email address is already in use by another user.', 'error');
        } else {
            // Update user
        if (!empty($password)) {
            $hashed = password_hash($password, PASSWORD_DEFAULT);
            $stmt = $pdo->prepare("UPDATE users SET email = ?, password = ?, first_name = ?, last_name = ?, phone = ?, address = ?, city = ? WHERE id = ?");
            $stmt->execute([$email, $hashed, $firstName, $lastName, $phone, $address, $city, $userId]);
        } else {
            $stmt = $pdo->prepare("UPDATE users SET email = ?, first_name = ?, last_name = ?, phone = ?, address = ?, city = ? WHERE id = ?");
            $stmt->execute([$email, $firstName, $lastName, $phone, $address, $city, $userId]);
        }
        ...
        <form method="POST">
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Phone</label>
                        <input type="text" name="phone" value="<?php echo htmlspecialchars($user['phone']); ?>" placeholder="e.g., +256771331531">
